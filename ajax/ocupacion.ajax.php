...
class AjaxOcupacion
{
    ...
    public function ajaxTraerCliente()
    {
        ...
        $clientes = ControladorClientes::ctrBuscarClientesPorIdentificacion($item, $valor);

        $results = [];
        foreach ($clientes as $cliente) {
            $results[] = [
                "id" => $cliente["cliId"],
                "identification" => $cliente["cliIdentificacion"],
                "phonePrimary" => $cliente["cliTelefono"],
                "email" => $cliente["cliCorreo"],
                "address" => $cliente["dirDireccion"],
                "name" => trim($cliente["cliPrimerNombre"] . " " . $cliente["cliSegundoNombre"] . " " . $cliente["cliPrimerApellido"] . " " . $cliente["cliSegundoApellido"]),
            ];
        }
